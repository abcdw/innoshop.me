...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while(any(row_tuple)):
           ...
           # Product.objects.filter(SKU=SKU).update(price=price,is_stock_empty=is_stock_empty)
            p = Product.objects.filter(SKU=SKU)
            if any(p):
                p = p[0]
                self.stdout.write("{0} {1} {2} {3}".format(SKU,p.actual_price,p.price,price))
            else:
                no_in_base = no_in_base +1
            try:
                row_tuple = read_sheet.row_values(row,start_colx=4,end_colx=10)
            except Exception as e:
                break
            row = row +1
        self.stdout.write("No in db: {0}\nErrors: {1}\n".format(no_in_base,errors))
